...
function isValidAddress(address) {
  ...
  return /^[0-9a-f]{40}$/.test(address); //address must be 40 characters
}
...
const isoDateRegEx = /^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}\.[0-9]{2,6}Z$/;

function isValidDate(dateISO) {
  if (typeof dateISO !== "string") return false;
  if (!isoDateRegEx.test(dateISO)) return false;
